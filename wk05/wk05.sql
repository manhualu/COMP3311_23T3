...
;
    ...
    -- From the parts select max(price)
select  y1.pid 
...
select sqr('5'); --> MAYBE, version dependant
...
-- substr(str, i, 1) extracts a substring from a string start at position i, extract 1 character
...
begin 
    ...
    return; -- need this to end the function
end; 
$$ language plpgsql;
...
declare 
    ...
    r           record;
begin 
    -- can also use the found variable to handle no bars in address e.g.
    -- select  *
    -- from    Bars b 
    -- where   b.address = _addr;
...
-- c. return employee names earning more than specified salary
create or replace function 
...
-- d. return details of highly paid employees
create or replace function 
